...
export function updateChildStatus(states:Map<string,ChildState>, id:string, name:string|undefined, status:string):TranscriptEntry[]{
 ...
 if(!old||(!terminal&&['completed','failed','interrupted'].includes(old.status))){
  ...
  entries.push({_id:crypto.randomUUID(),createdAt:Date.now(),kind:'tool_call',tool:{kind:'tool',toolKind:'subagent_task',toolName:'子代理',toolId:state.toolId,input:{subagentType:state.name}}})
